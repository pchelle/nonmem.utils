...
base_vpc_plot <- function(data,
  ...
  if (type %in% "blq") {
    tp_plot <- ggplot(data = vpc_data, mapping = aes(x = .data[["time"]])) +
      ...
      geom_line(aes(y = .data[["sim_min_med"]], color = sim_legend)) +
      geom_line(aes(y = .data[["sim_max_med"]], color = sim_legend)) +
      geom_line(aes(y = .data[["sim_med_med"]], color = sim_legend)) +
      geom_line(aes(y = .data[["obs_min"]], color = obs_legend)) +
      geom_line(aes(y = .data[["obs_max"]], color = obs_legend)) +
      geom_line(aes(y = .data[["obs_med"]], color = obs_legend)) +
  ...
  tp_plot <- ggplot(data = vpc_data, mapping = aes(x = .data[["time"]])) +
    ...
    geom_ribbon(
      aes(ymin = .data[["sim_min_min"]], ymax = .data[["sim_min_max"]], fill = sim_legend),
      alpha = 0.6
    ) +
    geom_ribbon(
      aes(ymin = .data[["sim_max_min"]], ymax = .data[["sim_max_max"]], fill = sim_legend),
      alpha = 0.6
    ) +
    geom_ribbon(
      aes(ymin = .data[["sim_med_min"]], ymax = .data[["sim_med_max"]], fill = sim_legend),
      alpha = 0.6
    ) +
    geom_line(aes(y = .data[["sim_min_med"]], color = sim_legend)) +
    geom_line(aes(y = .data[["sim_max_med"]], color = sim_legend)) +
